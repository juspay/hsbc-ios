...
+ (NSDictionary *)updateServiceNameInPayload:(NSDictionary *)payload tenantServiceName:(NSString *)tenantServiceName isResponse:(BOOL)isResponse {
    ...
    if (serviceName) {
        ...
        if (isResponse && ![tenantServiceName containsString:HSBC_HYPER_SERVICE_PREFIX]) {
            serviceName = [serviceName stringByReplacingOccurrencesOfString:HSBC_HYPER_SERVICE_PREFIX withString:@""];
            serviceNameChanged = true;
        } else if (![serviceName containsString:HSBC_HYPER_SERVICE_PREFIX]) {
            serviceName = [NSString stringWithFormat:@"%@%@", HSBC_HYPER_SERVICE_PREFIX, serviceName];
            serviceNameChanged = true;
        }
