...
- (instancetype)init
{
    HSBCPaymentsTenantMap *defaultTenant = [HSBCPaymentsTenantMap tenantWithName:@"DEFAULT"];
    
    HyperTenantParams *tenantParams = [[HyperTenantParams alloc] init];
    tenantParams.tenantId = defaultTenant.tenantId;
    tenantParams.releaseConfigURL = defaultTenant.releaseConfigTemplateUrl;
...
- (instancetype)initWithClientId:(NSString *)clientId {
    return [self initWithTenantId:@"DEFAULT" clientId:clientId];
}

- (instancetype)initWithTenantId:(NSString *)tenantId clientId:(NSString *)clientId {
    HSBCPaymentsTenantMap *tenant = [HSBCPaymentsTenantMap tenantWithName:tenantId];
    
    if (!tenant) {
        NSLog(@"Tenant '%@' not found, falling back to DEFAULT", tenantId);
        tenant = [HSBCPaymentsTenantMap tenantWithName:@"DEFAULT"];
    }
